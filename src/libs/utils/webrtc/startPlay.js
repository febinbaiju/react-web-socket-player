...
const peerConnectionSetRemoteDescriptionSuccess = (
  description,
  playSettings,
  websocket,
  peerConnection,
    ...
    .then(() => {
      websocket.send(
        '{"direction":"play", "command":"sendResponse", "streamInfo":' +
          JSON.stringify(getStreamInfo(playSettings)) +
          ', "sdp":' +
          JSON.stringify(description) +
          ', "userData":' +
          JSON.stringify(getUserData(playSettings)) +
          "}"
      );
    })
    .catch((error) => {
      ...
      peerConnection
        ...
        .then(() =>
          peerConnection
            .createAnswer()
            .then((description) =>
              peerConnectionSetRemoteDescriptionSuccess(
                description,
                playSettings,
                websocket,
                peerConnection,
